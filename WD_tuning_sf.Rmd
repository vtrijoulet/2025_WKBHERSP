...
Note that despite the residuals needing improving, the retrospective patterns are already much better than with the current assessment model.
...
## Bias in catch and survey fleets due to parameter coupling (keyLogSta and keylogFpar parameters, respectively)

From Figure \ref{fig:resdeffit}, we see medium high significant (darker blue) bias in the IBTS/BITSQ1 fleet (fleet 5) for ages 4 and 5. The bias is also visible in the bubble plot, where we see age effects (only red or blue residuals for most of the time series) for both ages. As a result, we choose to uncoupled these ages as follows:
...
```{r catchfit10, echo=FALSE, fig.cap="Retrospective patterns for the fit 'fit10'.", fig.dim=c(6,6)}
...
Despite the fit showing slightly better results, to favour model simplicity, we go ahead with the simpler model. We therefore go ahead with "fit9" that will be likely more robust. 
...
```{r matplot, echo=FALSE, fig.cap="Retrospective patterns for the fit 'fit9'.", fig.dim=c(6,6)}
